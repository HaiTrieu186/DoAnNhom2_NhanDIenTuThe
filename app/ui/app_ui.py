...
def create_ui_widgets(root, app_commands):
    ...
    # --- Style ---
    style = ttk.Style()
    style.theme_use('vista') # Thử dùng theme 'vista', 'clam', 'alt', 'default', 'classic'
    ...
    style.configure('Result.TLabel', font=('Segoe UI', 14, 'italic'), foreground="navy", background="#F0F0F0") # Đổi màu kết quả
    style.configure('Path.TLabel', font=('Segoe UI', 9), background="#F0F0F0", foreground="dimgray") # Đổi màu đường dẫn
    ...
    # --- Khung hiển thị ảnh ---
    image_label = tk.Label(main_frame, bg="darkgrey") # Nền xám đậm hơn
    ...
    filepath_label = ttk.Label(upload_frame, text="Chưa chọn file", style='Path.TLabel', width=60, anchor=tk.W, relief=tk.SUNKEN, padding=3) # Dùng SUNKEN
    ...
    ttk.Frame(camera_frame).pack(side=tk.LEFT, expand=True, fill=tk.X) # Khung đẩy nút Thoát
